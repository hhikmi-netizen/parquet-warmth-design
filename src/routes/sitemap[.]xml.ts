...
const STATIC_ROUTES: { path: string; priority: number; changefreq: string }[] = [
  ...
  { path: "/estimer-mon-parquet", priority: 0.9, changefreq: "monthly" },
  ...
  { path: "/prix-poncage-parquet", priority: 0.95, changefreq: "monthly" },
  { path: "/parquet-flottant-ou-massif", priority: 0.95, changefreq: "monthly" },
  { path: "/renover-parquet-ancien", priority: 0.95, changefreq: "monthly" },
  { path: "/parquet-sur-carrelage", priority: 0.95, changefreq: "monthly" },
  ...
  { path: "/devenir-artisan/inscription", priority: 0.5, changefreq: "monthly" },
  ...
  server: {
    handlers: {
      GET: async () => {
        ...
        for (const g of GONDOLAGE_CASES) {
          urls.push(`<url><loc>${BASE}/parquet-qui-gondole/${g.slug}</loc><lastmod>${today}</lastmod><changefreq>monthly</changefreq><priority>0.85</priority></url>`);
        }
